...
export class SpotifyClient {
  ...
  private toError(response: Response): AppError { const code = response.status === 401 ? 'SPOTIFY_NOT_AUTHENTICATED' : response.status === 403 ? 'PLAYLIST_FORBIDDEN' : response.status === 404 ? 'PLAYLIST_NOT_FOUND' : response.status === 429 ? 'SPOTIFY_RATE_LIMIT' : 'SPOTIFY_API_ERROR'; return new AppError(code, `A API do Spotify respondeu com erro (${response.status}).`, undefined, response.status); }
}
