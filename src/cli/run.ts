...
async function menu(deps: Deps): Promise<void> {
  while (true) {
    ...
    if (action === 'playlists') console.log(await deps.exports.playlistsToJson());
    if (action === 'list') console.table((await deps.playlists.listPlaylists()).map((p) => ({ name: p.name, id: p.id, items: p.totalItems })));
    if (action === 'items') { const playlist = await choosePlaylist(deps); if (playlist) console.log(await deps.exports.playlistItemsToJson(playlist)); }
    if (action === 'create') { const name = await input({ message: 'Nome da playlist:' }); const description = await input({ message: 'Descrição (opcional):' }); if (await confirm({ message: `Criar "${name}"?` })) console.log(await deps.playlists.create({ name, description })); }
